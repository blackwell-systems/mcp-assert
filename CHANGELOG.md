...
- **Trajectory assertions** — validate that agents call MCP tools in the correct order. Essential for skill protocol compliance. 4 assertion types: `order` (tools appear in sequence), `presence` (all listed tools called), `absence` (tools NOT called), `args_contain` (tool called with specific argument values). Inline trace (YAML) and audit log (JSONL) sources. Runs in 0ms (no server needed). 3 example assertions for agent-lsp skills: rename protocol (prepare_rename before rename_symbol), safe-edit protocol (simulate before apply_edit), refactor protocol (blast radius before edits).
