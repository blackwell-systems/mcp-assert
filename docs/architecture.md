...
The codebase is organized into four packages under `internal/`, plus the entry point.

### `cmd/mcp-assert/main.go`

The binary entry point. It reads the first CLI argument and dispatches to the appropriate function in the `runner` package via a command registry map (`map[string]func([]string) error`). Commands include `Audit`, `Fuzz`, `Run`, `Matrix`, `CI`, `Init`, `Coverage`, `Generate`, `Snapshot`, `Watch`, and `Intercept`. This file also defines `printUsage()` for help text and exposes a `Version` variable set at build time.
...
| `fuzz.go` | `Fuzz()`: adversarial input testing. Same connection/discovery flow as audit, but calls each tool N times with category-based adversarial inputs generated by the `fuzz` package. Classifies results as passed/crash/timeout/protocol. Supports `--seed` for reproducibility and `--tool` for single-tool targeting. |
...
### `internal/fuzz/` (adversarial input generation)

This package generates category-based adversarial inputs from MCP tool JSON Schemas. It has no I/O and no dependencies beyond `mcp-go/mcp` for the `ToolInputSchema` type.

| File | Responsibility |
|------|----------------|
| `generator.go` | `GenerateInputs()`: produces a slice of `InputCase` (label + args) for a given tool schema and seed. Structural cases first (empty object, null args, missing required fields), then per-property type-specific cases (string, number, boolean, array, object), then seeded random mutations to fill remaining slots. |

**Input generation strategy:** Rather than pure random fuzzing (which produces garbage that gets rejected at schema validation), the generator produces inputs that are *almost valid*: correct structure but wrong values. These are more likely to slip past schema validation and reach the tool's handler logic, where crashes actually happen.

**Categories per type:**

- **String:** empty, whitespace, 10KB, null bytes, unicode, newlines, path traversal, shell injection, SQL injection
- **Number:** zero, negative, MAX_INT, MIN_INT, float precision, negative zero
- **Boolean:** null, 0, 1, "true", "false" (type coercion traps)
- **Array:** empty, null, 1000 elements, mixed types, nested
- **Object:** empty, null, 20 levels deep, 100 keys
- **Cross-property:** omit each required field one at a time
...
| `fuzz.go` | `PrintFuzzHeader()`, `PrintFuzzToolResult()`, `PrintFuzzSummary()`: fuzz-specific report formatting with per-tool pass/fail counts, failure category breakdown (CRASH/HANG/PROTO), and reproduction command. |
