...
| **`init` one-step suite generation** | Medium | `mcp-assert init --server <cmd>` runs `generate` + `snapshot --update` in one command. Complete working suite with 100% tool coverage, zero manual assertion writing. |
